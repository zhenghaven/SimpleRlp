...
static void TestEncodePrimitiveIntValue(
	size_t val,
	size_t expWidth,
	const std::vector<uint8_t>& expBytes
)
{
	using PriEncoder =
		EncodePrimitiveIntValue<size_t, Endian::native, false>;

	auto width = PriEncoder::EncodedWidth(val);
	EXPECT_EQ(width, expWidth);

	std::vector<uint8_t> bytes;
	PriEncoder::Encode(bytes, val);
	EXPECT_EQ(bytes, expBytes);
}

GTEST_TEST(TestRlpWriter, EncodePrimitiveIntValue)
{
	TestEncodePrimitiveIntValue(
		0x00U,
		0U,
		{ }
	);
...
GTEST_TEST(TestRlpWriter, TwoWaysConvertTest_B01)
{
	...
	std::vector<uint8_t> expRlp = { 0x00U };

	auto rlp = WriteRlp(testObj);
	EXPECT_EQ(rlp, expRlp);

	auto rlpSize = CalcRlpSize(testObj);
...
GTEST_TEST(TestRlpWriter, TwoWaysConvertTest_B02)
{
	...
	std::vector<uint8_t> expRlp = { 0x81U, 0xFFU };
...
static const SimpleObjects::Bytes& GetTestLongBytes()
{
...
static const std::vector<uint8_t>& GetTestLongBytesExpected()
{
	static std::vector<uint8_t> inst({
		0xB9U, // Long bytes
			0x01U, 0x00U, // len = 0x0100 = 256
		0x00U, 0x01U, 0x02U, 0x03U, 0x04U, 0x05U, 0x06U, 0x07U, 0x08U, 0x09U, 0x0AU, 0x0BU, 0x0CU, 0x0DU, 0x0EU, 0x0FU,
		0x10U, 0x11U, 0x12U, 0x13U, 0x14U, 0x15U, 0x16U, 0x17U, 0x18U, 0x19U, 0x1AU, 0x1BU, 0x1CU, 0x1DU, 0x1EU, 0x1FU,
		0x20U, 0x21U, 0x22U, 0x23U, 0x24U, 0x25U, 0x26U, 0x27U, 0x28U, 0x29U, 0x2AU, 0x2BU, 0x2CU, 0x2DU, 0x2EU, 0x2FU,
		0x30U, 0x31U, 0x32U, 0x33U, 0x34U, 0x35U, 0x36U, 0x37U, 0x38U, 0x39U, 0x3AU, 0x3BU, 0x3CU, 0x3DU, 0x3EU, 0x3FU,
...
GTEST_TEST(TestRlpWriter, TwoWaysConvertTest_B03)
{
	...
	std::vector<uint8_t> expRlp = GetTestLongBytesExpected();
...
GTEST_TEST(TestRlpWriter, TwoWaysConvertTest_L01)
{
	...
	std::vector<uint8_t> expRlp = {
		0xF9U, // Long list
			0x01U, 0x19U, // length = 0x0100 + 0x19 = 0x0119 = 156 + 25 = 281
	...
	expRlp.insert(expRlp.end(), GetTestLongBytesExpected().begin(), GetTestLongBytesExpected().end());
...
static const SimpleObjects::List& GetTestLongList()
{
	static SimpleObjects::List inst({
...
static const std::vector<uint8_t>& GetTestLongListExpected()
{
	static std::vector<uint8_t> inst({
		0xF8U, // Long list
			0x5AU, // Length = 90 = 0x5A
		0x00U, 0x01U, 0x02U, 0x00U, 0x01U, 0x02U, 0x00U, 0x01U, 0x02U, 0x00U, 0x01U, 0x02U, 0x00U, 0x01U, 0x02U,
		0x00U, 0x01U, 0x02U, 0x00U, 0x01U, 0x02U, 0x00U, 0x01U, 0x02U, 0x00U, 0x01U, 0x02U, 0x00U, 0x01U, 0x02U,
...
GTEST_TEST(TestRlpWriter, TwoWaysConvertTest_L02)
{
	...
	std::vector<uint8_t> expRlp = {
		0xF8U, // Long list
			0x64U, // Length = 8 + 92 = 100 = 0x64
		// Short list
		0xC0U,
		0xC6U,
			0x00U, 0x01U, 0x02U, 0x03U, 0x04U, 0x05U,
	};
	expRlp.insert(expRlp.end(), GetTestLongListExpected().begin(), GetTestLongListExpected().end());
						...
						0xC4U, // nestedList5
							0xC3U, // nestedList4
								0xC2U, // nestedList3
									0xC1U, // nestedList2
										0xC0U, // nestedList1
		// Insert GetTestLongListExpected here
		0x00U,
		0x86U,
			0x00U, 0x01U, 0x02U, 0x03U, 0x04U, 0x05U,
	};
	expRlp.insert(expRlp.end() - 8, GetTestLongListExpected().begin(), GetTestLongListExpected().end());
...
// {
// 	using Concatenator = OutContainerConcat<OutputContainerType>;
